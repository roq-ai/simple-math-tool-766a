...
import { getCalculationById } from 'apiSdk/calculations';
import { CalculationInterface } from 'interfaces/calculation';

function CalculationViewPage() {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('calculation', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/calculations/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                {hasAccess('math_tool', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Math Tool"
                    text={
                      <Link as={NextLink} href={`/math-tools/view/${data?.math_tool?.id}`}>
                        {data?.math_tool?.name}
                      </Link>
                    }
                  />
